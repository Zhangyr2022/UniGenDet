...
class PackedDataset(torch.utils.data.IterableDataset):
    ...
    def set_sequence_status(self):
        sequence_status = dict(
            ...
            label = list(),
            generation_auth_explanation_list = list(),
            gt_vit_list = list(),
            gt_prompt_list = list(),
            # # uid fake
            # vit_group_uids = list(), # vit uid vit_token_seqlens
            # vae_group_uids = list(), # vae uid vae_latent_shapes
            # vae_group_is_fake = list(), # vae_group_uids bool
        )
    ...
    def to_tensor(self, sequence_status):
        ...
        if len(sequence_status['vae_image_tensors']) > 0:
            ...
            # # vae uid fake
            # data['vae_group_uids'] = torch.tensor(sequence_status['vae_group_uids'], dtype=torch.long)
            # data['vae_group_is_fake'] = torch.tensor(sequence_status['vae_group_is_fake'], dtype=torch.bool)
        ...
        if len(sequence_status['packed_vit_tokens']) > 0:
            ...
            # # vit uid
            # data['vit_group_uids'] = torch.tensor(sequence_status['vit_group_uids'], dtype=torch.long)
        ...
        if ('generation_auth_explanation_list' in sequence_status.keys()):
            data['generation_auth_explanation_list'] = (sequence_status['generation_auth_explanation_list'])
        if ('gt_vit_list' in sequence_status.keys()):
            data['gt_vit_list'] = (sequence_status['gt_vit_list'])
        if ('gt_prompt_list' in sequence_status.keys()):
            data['gt_prompt_list'] = (sequence_status['gt_prompt_list'])
        data['label'] = torch.tensor(sequence_status['label'])
        return data
        ...
        while True:
                    ...
                    if self.is_mandatory[group_index]:
                        while True:
                            try:
                                ...
                                print("StopIteration")
                                # print(f"Re-creating iterator for mandatory group {group_index}", "total groups:", len(self.grouped_datasets))
                                # self.dataset_iters[group_index] = iter(self.grouped_datasets[group_index])
                                # iteration_end_flag = True
                                # # continue
                                # # group_iter = self.dataset_iters[group_index]
                                # # sample = next(group_iter)
                                # # break
                            # sample = next(group_iter)
            ...
            if sequence_status['curr'] < self.prefer_buffer_before and len(buffer) > 0:
                ...
                try:
                    ...
                    print(f"Error")
                    # print(f"Re-creating iterator for group {group_index}")
                    # # Re-create the iterator for the exhausted dataset and try again
                    # self.dataset_iters[group_index] = iter(self.grouped_datasets[group_index])
                    # # sample = next(self.dataset_iters[group_index])
                    # continue
    ...
    def pack_sequence(self, sample, sequence_status):
        vit_image_tensor_list = sample.get('vit_image_tensor_list', [])
        gen_image_tensor_list = sample.get('gen_image_tensor_list', [])
        # For backward compatibility
        if 'image_tensor_list' in sample:
            image_tensor_list = sample['image_tensor_list']
        else:
            image_tensor_list = vit_image_tensor_list + gen_image_tensor_list
        ...
        for item in sequence_plan:
            ...
            if item['type'] == 'text':
                ...
                image_tensor = vit_image_tensor_list.pop(0)
                ...
                # vit uid
                # sequence_status['vit_group_uids'].append(int(item.get('uid', len(sequence_status['vit_group_uids']))))
            ...
            # elif item['type'] == 'fake_vae_image':
            # image_tensor = gen_image_tensor_list.pop(0)
            ...
            # curr_rope_id += 1
                ...
                if item['enable_cfg'] == 1 and random.random() < self.data_config.vae_cond_dropout_prob:
                    ...
                    curr_rope_id += 1
                    continue
                if sample['data_indexes']['dataset_name'] == 't2i_pretrain':
                    sequence_status['generation_auth_explanation_list'].extend(sample["generation_auth_explanation_list"])
                    sequence_status['gt_vit_list'].extend(sample["gt_vit_list"])
                    sequence_status['gt_prompt_list'].extend(sample["gt_prompt_list"])
                # add a <|startofimage|> token
                sequence_status['packed_text_ids'].append(self.start_of_image)
                sequence_status['packed_text_indexes'].append(curr)
                curr += 1
        ...
        if not self.use_flex:
            # print([item['type'] for item in sequence_plan])
            # print(split_lens, attn_modes)
        ...
        if 'label' in sample:
            sequence_status['label'].append(sample['label'])
            # print("append label:", sample['label'])
        return sequence_status
...
class SimpleCustomBatch:
    def __init__(self, batch):
        ...
        if 'generation_auth_explanation_list' in data.keys():
            self.generation_auth_explanation_list = data['generation_auth_explanation_list']
        if 'gt_vit_list' in data.keys():
            self.gt_vit_list = data['gt_vit_list']
        if 'gt_prompt_list' in data.keys():
            self.gt_prompt_list = data['gt_prompt_list']
        # if "vae_group_uids" in data.keys():
        # self.vae_group_uids = data["vae_group_uids"]
        # self.vae_group_is_fake = data["vae_group_is_fake"]
        # if "vit_group_uids" in data.keys():
        # self.vit_group_uids = data["vit_group_uids"]
    ...
    def pin_memory(self):
        ...
        # if hasattr(self, 'generation_auth_explanation_list'):
        # self.generation_auth_explanation_list = self.generation_auth_explanation_list.pin_memory()
        # if hasattr(self, 'vae_group_uids'):
        # self.vae_group_uids = self.vae_group_uids.pin_memory()
        # self.vae_group_is_fake = self.vae_group_is_fake.pin_memory()
        # if hasattr(self, 'vit_group_uids'):
        # self.vit_group_uids = self.vit_group_uids.pin_memory()
        return self
    ...
    def cuda(self, device):
        ...
        # if hasattr(self, 'vae_group_uids'):
        # self.vae_group_uids = self.vae_group_uids.to(device)
        # self.vae_group_is_fake = self.vae_group_is_fake.to(device)
        # if hasattr(self, 'vit_group_uids'):
        # self.vit_group_uids = self.vit_group_uids.to(device)
        return self
    ...
    def to_dict(self):
        ...
        # if hasattr(self, 'vae_group_uids'):
        # data['vae_group_uids'] = self.vae_group_uids
        # data['vae_group_is_fake'] = self.vae_group_is_fake
        # if hasattr(self, 'vit_group_uids'):
        # data['vit_group_uids'] = self.vit_group_uids
        if hasattr(self, 'generation_auth_explanation_list'):
            data['generation_auth_explanation_list'] = self.generation_auth_explanation_list
        if hasattr(self, 'gt_vit_list'):
            data['gt_vit_list'] = self.gt_vit_list
        if hasattr(self, 'gt_prompt_list'):
            data['gt_prompt_list'] = self.gt_prompt_list
