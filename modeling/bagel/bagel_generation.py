...
class BagelConfigGeneration(PretrainedConfig):
    def __init__(
        self,
        visual_gen=True,
        visual_und=True,
...
class BagelGeneration(PreTrainedModel):
    config_class = BagelConfigGeneration
    base_model_prefix = 'bagel'

    def __init__(self, language_model, vit_model, config: BagelConfigGeneration, vae_model=None):
    ...
    def forward(
        ...
        if (
            ...
            for i in range(num_gen_imgs):
                ...
                vae_height, vae_width = img_tensor.shape[1:]
            ...
            for i, (h, w) in zip(range(len(included_samples)), sequence_status['patchified_vae_latent_shapes']):
                ...
                latent = torch.einsum("chpwq->hwpqc", latent).reshape(-1, p * p * self.latent_channel)
                packed_latent.append(latent)
            # padded_latent = global_vae_model.encode(packed_sequence.pop('padded_images'))
            # p = self.latent_patch_size
            # packed_latent = []
            ...
            packed_ce_preds = self.language_model.lm_head(last_hidden_state[sequence_status['ce_loss_indexes']])
            # print(sequence_status['packed_label_ids'])
            det_ce = F.cross_entropy(packed_ce_preds, sequence_status['packed_label_ids'], reduction="none")
            # print("ce", ce)
        ...
        return dict(mse=mse, ce=ce, det_ce=det_ce, det_ce_loss_indexes=sequence_status['ce_loss_indexes']), fake_predicted
