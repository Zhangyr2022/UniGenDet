...
class BagelConfigGenerationDIGA(PretrainedConfig):
    def __init__(
        self,
        visual_gen=True,
        visual_und=True,
...
class BagelGenerationDIGA(PreTrainedModel):
    config_class = BagelConfigGenerationDIGA
    base_model_prefix = 'bagel'

    def __init__(self, language_model, vit_model, config: BagelConfigGenerationDIGA, vae_model=None):
        ...
        # self.diga_norm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        generation_diga_hidden_states, last_hidden_state = self.language_model.forward_diga(
        ...
        if (
            ...
            vit_tokens_det = det_diga_hidden_states[sequence_status['packed_vit_token_indexes']]
            ...
            latent_tokens_gen = generation_diga_hidden_states[packed_vae_token_indexes]
            processed_latent_tokens = self.diga_mlp(latent_tokens_gen)
            ...
            diga_loss = cos_sim_loss

            # --- End of requested modification ---

        ce = None
        return dict(mse=mse, ce=ce, diga_loss=diga_loss)
        # return dict(mse=mse, ce=ce), None


    def pdigare_prompts(self, curr_kvlens, curr_rope, prompts, tokenizer, new_token_ids):
    ...
    def pdigare_vit_images(self, curr_kvlens, curr_rope, images, transforms, new_token_ids):
    ...
    def pdigare_vae_images(self, curr_kvlens, curr_rope, images, transforms, new_token_ids, timestep=0):
    ...
    def pdigare_vae_latent(self, curr_kvlens, curr_rope, image_sizes, new_token_ids):
        packed_text_ids, packed_text_indexes = list(), list()
    ...
    def pdigare_vae_latent_cfg(self, curr_kvlens, curr_rope, image_sizes):
    ...
    def pdigare_start_tokens(self, curr_kvlens, curr_rope, new_token_ids):
    ...
    def chat(
        ...
        # add images
        for image in images:
            generation_input, newlens, new_rope = self.pdigare_vit_images(
                curr_kvlens=newlens,
                curr_rope=new_rope,
                images=[image],
        ...
        # add text
        generation_input, newlens, new_rope = self.pdigare_prompts(
            curr_kvlens=newlens,
            curr_rope=new_rope,
            prompts=[prompt],
            tokenizer=tokenizer,
        ...
        # decode
        generation_input = self.pdigare_start_tokens(newlens, new_rope, new_token_ids)
        for k, v in generation_input.items():
            if torch.is_tensor(v):
                generation_input[k] = v.to(device)
        with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
