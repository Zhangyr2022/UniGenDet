...
from modeling.bagel import (
    BagelConfigGenerationDIGA, BagelGenerationDIGA, Qwen2Config, Qwen2ForCausalLM, SiglipVisionConfig, SiglipVisionModel
)
...
class TrainingArguments:
    ...
    det_ce_weight: float = field(
        # default=0.2,
        default=1.0,
        metadata={"help": "Scaling factor for the deterministic cross-entropy loss term."}
    )
    diga_weight: float = field(
        default=0.5,
        metadata={"help": "Scaling factor for the DIGA loss term."}
    )
...
def main():
    ...
    config = BagelConfigGenerationDIGA(
    ...
    if training_args.freeze_llm:
        ...
        for param in model.parameters():
            param.requires_grad = False
        for param in model.repa_mlp.parameters():
            param.requires_grad = True
    if training_args.freeze_vit:
        model.vit_model.eval()
        for param in model.vit_model.parameters():
            param.requires_grad = False
    if training_args.freeze_det:
    ...
    # Print all trainable parameter names for debugging and reproducibility.
    ...
    for curr_step, data in enumerate(train_loader, start=train_step):
        try:
            ...
            data.update({'dataset':train_dataset})
            with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
                if training_args.visual_gen:
                    with torch.no_grad():
                        data['padded_latent'] = vae_model.encode(data.pop('padded_images'))
                loss_dict, fake_predicted = fsdp_model(**data)
            loss = 0
            ce = loss_dict["ce"]
            if ce is not None:
            ...
            if loss_dict.get("det_ce", None) is not None:
                total_det_ce_tokens = torch.tensor(len(loss_dict['det_ce_loss_indexes']), device=device)
                dist.all_reduce(total_det_ce_tokens, op=dist.ReduceOp.SUM)
                if training_args.ce_loss_reweighting:
                    det_ce = det_ce * ce_loss_weights
                    total_ce_loss_weights = ce_loss_weights.sum()
                    dist.all_reduce(total_ce_loss_weights, op=dist.ReduceOp.SUM)
                    det_ce = det_ce.sum() * dist.get_world_size() / total_ce_loss_weights
                else:
                    det_ce = det_ce.sum() * dist.get_world_size() / total_det_ce_tokens
                loss_dict["det_ce"] = det_ce.detach()
                loss = loss + det_ce * training_args.det_ce_weight
            diga_loss = loss_dict.get("repa_loss", None)
            if diga_loss is not None:
                loss_dict["diga_loss"] = diga_loss.detach()
                loss = loss + diga_loss * training_args.diga_weight
                loss_dict.pop("repa_loss", None)
            ...
            import traceback
            traceback.print_exc()
            logger.error(f"Encountered an error at step {curr_step}: {e}")
            torch.cuda.empty_cache()
            dist.barrier()
            logger.info(f"Skipping step {curr_step} due to the error and continuing training.")
            continue
