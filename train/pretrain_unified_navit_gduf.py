...
def main():
    ...
    for curr_step, data in enumerate(train_loader, start=train_step):
        try:
            ...
            if fake_predicted is not None and len(label) > 0:
                label_cross_entropy_loss = criterion(fake_predicted.to(device),label.to(device))
                loss = loss + label_cross_entropy_loss * training_args.label_weight
                loss_dict["label_cross_entropy"] = label_cross_entropy_loss.detach()
